...
setwd("E:/DWCT/M_Analysis_Ajit_Sir/GSE148353/June_Run/June_Run_Micro_TOM_EX")
...
gene_expression_data1 <- read.csv("Count_Data_Run.csv", row.names = 1)
gene_expression_data1 <- gene_expression_data1 [10:18]
# Transpose the gene expression data
gene_expression_data <- t(gene_expression_data1)

# Example metadata (assuming you are reading from CSV files)
metadata1 <- read.csv("Meta_Data_Run.csv")
metadata <- metadata1 [10:18,]
# Check dimensions
...
#load the count data
count_data1 <- read.csv("Count_Data_Run.csv", header=TRUE,row.names = 1)
count_data <- gene_expression_data1
colnames(count_data)
head(count_data)

#load the sample info
sample_info1 <- read.csv("Meta_Data_Run.csv", header = TRUE,row.names = 1)
...
# Example gene expression data (assuming you are reading from CSV files)
gene_expression_data1 <- read.csv("Normalized_Salt stress_vs_control_June.csv", row.names = 1)
gene_expression_data1 <- gene_expression_data1 [1:9]
# Transpose the gene expression data
gene_expression_data <- t(gene_expression_data1)
...
ggplot(umap_df, aes(x = X1, y = X2, color = Time)) +
  geom_point(size = 3) +
  stat_ellipse(aes(group = Time), type = "norm", linetype = 1, size = 1) +
  labs(title = "GSE148353_UMAP",
       x = "UMAP Component 1", y = "UMAP Component 2") +
  theme_minimal()
