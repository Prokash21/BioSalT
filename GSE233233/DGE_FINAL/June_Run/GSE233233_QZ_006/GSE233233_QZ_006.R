...
ggplot(umap_df, aes(x = X1, y = X2, color = agent)) +
  geom_point(size = 3) +
  geom_text(aes(label = rownames(umap_df)), size = 2, nudge_y = 0.1) +
...
label_colors <- as.numeric(factor(htree$labels))

# Plot the dendrogram with colored labels
plot(htree, labels = metadata$Sample.Name, main = "Hierarchical Clustering Dendrogram", col = label_colors)

#######
# Plot hierarchical clustering with colors based on metadata groups
plot(htree, labels = metadata$Sample.Name, main = "Hierarchical Clustering Dendrogram")
rect.hclust(htree, k = length(unique(metadata$group)), border = 2:5)


# Adjust `k` to a valid number of clusters (usually the number of unique groups)
k_clusters <- length(unique(metadata$group))
if (k_clusters > 1) {
  rect.hclust(htree, k = k_clusters, border = 2:5)
} else {
  warning("Only one cluster detected. Unable to draw rectangles.")
}
...
group_df <- data.frame(label = htree$labels, group = factor(groups))

# Visualize the phylogenetic tree with colors
ggtree(htree) %<+% group_df +
...
data68<- data[,-11]
#metadata <- read.csv("Meta_Data_final.csv", header = TRUE, row.names = 1) 
metadata68 <- metadata [-11,]
# Quality Control: detect outlier genes
gsg <- goodSamplesGenes(t(data68))
summary(gsg)
...
# Detect outlier samples using hierarchical clustering
htree <- hclust(dist(t(data68)), method = "average")
######
# Assuming 'htree' and 'metadata' are correctly defined
...
# Plot the dendrogram with colored labels
plot(htree, labels = metadata68$Sample.Name, main = "Hierarchical Clustering Dendrogram", col = label_colors)

#######
# Plot hierarchical clustering with colors based on metadata groups
plot(htree, labels = metadata68$Sample.Name, main = "Hierarchical Clustering Dendrogram")

####################################################*********************************************

groups <- rep(metadata68$agent)
...
# Remove 59 control ,,,,, Remove 6h 61,62,63
...
count_data <- read.csv("Count_Data_final.csv", header=TRUE,row.names = 1)
count_data <- count_data[1:18]
count_data <- count_data[, -c(2, 4, 5, 6)]
colnames(count_data)
head(count_data)
...
sample_info <- read.csv("Meta_Data_final.csv", header = TRUE,row.names = 1)
sample_info <- sample_info[1:18,]
sample_info <- sample_info [-c(2, 4, 5, 6),]
colnames(sample_info)
head(sample_info)
...
head(normalize_counts)
dim(normalize_counts)
write.csv(normalize_counts,"Remove_4_Normalized_Salt_vs_control.csv")

# After removing 4 samples
######################################### UMAP , PCA ###############################

# Example gene expression data (assuming you are reading from CSV files)
gene_expression_data1 <- read.csv("Remove_4_Normalized_Salt_vs_control.csv", row.names = 1)
gene_expression_data2 <- gene_expression_data1 [1:14]
# Transpose the gene expression data
gene_expression_data <- t(gene_expression_data2)
...
data <- read.csv("Remove_4_Normalized_Salt_vs_control.csv", header = TRUE, row.names = 1)
...
count_data1 <- read.csv("Count_Data_final.csv", header=TRUE,row.names = 1)
count_data2 <- count_data1[1:18]
count_data <- count_data2[, -c(2, 4, 5, 6, 11)]
colnames(count_data)
head(count_data)
...
sample_info <- read.csv("Meta_Data_final.csv", header = TRUE,row.names = 1)
sample_info <- sample_info[1:18,]
sample_info <- sample_info [-c(2, 4, 5, 6, 11),]
colnames(sample_info)
head(sample_info)
...
head(normalize_counts)
dim(normalize_counts)
write.csv(normalize_counts,"Remove_5_Normalized_Salt_vs_control.csv")

# After removing 4 samples
######################################### UMAP , PCA ###############################

# Example gene expression data (assuming you are reading from CSV files)
gene_expression_data1 <- read.csv("Remove_5_Normalized_Salt_vs_control.csv", row.names = 1)
gene_expression_data2 <- gene_expression_data1 [1:13]
# Transpose the gene expression data
gene_expression_data <- t(gene_expression_data2)
...
data <- read.csv("Remove_5_Normalized_Salt_vs_control.csv", header = TRUE, row.names = 1)
...
write.csv(resLFC_p_cut, file = "Remove_5_resLFC_p_cut.csv")
...
write.csv(top_UP_hits1, file = "Remove_5_UP_Salt_vs_control.csv", row.names = TRUE)

Downregulated <- resLFC[resLFC$log2FoldChange < -1 & resLFC$padj < 0.05, ]
top_hits1 <- Downregulated[order(Downregulated$padj),]
write.csv(top_hits1, file = "Remove_5_DOWN_agent_Salt_vs_control.csv", row.names = TRUE )
